...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		// why, we're bootstrapping the first node in the server and it will be our leader
		if i != 0 {
			err = logs[0].Join(
	...
	for _, record := range records {
		// Why? We are appending logs to our leader and see if it gets replicated
		// to our nodes within our cluster
